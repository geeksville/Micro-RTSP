...
{
    memset( m_URLPresentationPart, 0x00, sizeof( m_URLPresentationPart ) );
    memset( m_URLStreamPart,    0x00, sizeof( m_URLStreamPart ) );
...
{
    while ( *buf  && ( *buf == ' ' || *buf == '\t' ) ) // skipping space after ':'
        ++buf;
...
bool CRtspSession::ParseRtspRequest(char * aRequest, unsigned aRequestSize)
{
    ...
    */
    ...
    while ( *cur_pos != ' ' && *cur_pos != '\t' ) // skip possible CRLF and command name as we alredy got it in the handleRequests()
    {
    ...
    // getting host:port
    for( ; ! isspace ( *cur_pos ) && *cur_pos != '/'; ++cur_pos, ++dst_pos )
    {
        if ( dst_pos == MAX_HOSTNAME_LEN )
            return false;

        m_URLHostPort[ dst_pos ] = *cur_pos;
    }

    if ( *cur_pos != '/' ) // no next part
        return false;

    m_URLHostPort[ dst_pos ] = '\0';
    ...
    for( dst_pos = 0; ! isspace ( *cur_pos ) && *cur_pos != '/'; ++cur_pos, ++dst_pos )
    {
        if ( dst_pos == RTSP_PARAM_STRING_MAX )
            return false;

        m_URLPresentationPart[ dst_pos ] = *cur_pos;
    }

    if ( *cur_pos != '/' ) // no next part
        return false;

    m_URLPresentationPart[ dst_pos ] = '\0';
    if ( debug ) printf( "+ pres: %s\n", m_URLPresentationPart );
    ...
    for( dst_pos = 0; ! isspace ( *cur_pos ) && *cur_pos != '/'; ++cur_pos, ++dst_pos )
    {
        ...
        m_URLStreamPart[ dst_pos ] = *cur_pos;
    }
    ...
    for(;;)
    {
        ...
        // we're not using isspace here to detect a CRLF
        if ( *cur_pos == ' ' || *cur_pos == '\t' ) // this is either tail or continuation, so skipping anything till CRLF
        {
        ...
        for ( char *p = cur_pos; *p; ++p )
        {
            ...
            if ( *p == '\r' && p[ 1 ] == '\n' )
            {
                if ( p[ 2 ] != ' ' && p[ 2 ] != '\t' ) // solid. ending search
                    break;
        ...
        if ( m_RtspCmdType == RTSP_SETUP && 0 == strncmp( "Transport:", cur_pos, 10 ) )
        {
            ...
            if ( 0 != strncmp( cur_pos, "RTP/AVP", 7) )
                return false;
            
            cur_pos += 7;

            if ( 0 == strncmp( cur_pos, "/TCP", 4 ) )
            {
                m_TcpTransport = true;
                cur_pos += 4;
            }
            ...
            for(;;)
            {
                ...
                next_part = strpbrk( cur_pos, ";\r" );
                last_char = *next_part; // in case we'll need to put \0 here

                if ( m_ClientRTPPort == -1 && 0 == strncmp( cur_pos, "client_port", 11 ) ) // "client_port" "=" port [ "-" port ]
                {
                    char *p = ( cur_pos += 12 );
                    while( isdigit( *p ) )
                        ++p;
...
void CRtspSession::Handle_RtspDESCRIBE()
{
    ...
    if ((strcmp(m_URLPresentationPart,"mjpeg") == 0) && (strcmp(m_URLStreamPart,"1") == 0)) m_StreamID = 0; else
    if ((strcmp(m_URLPresentationPart,"mjpeg") == 0) && (strcmp(m_URLStreamPart,"2") == 0)) m_StreamID = 1;
    if (m_StreamID == -1)
